...
//    }
    ...
    public void setValue(TextArea textarea_result, String Encode_type, String souce_text, String input_encoding,String ouput_encoding, Boolean is_line) {
        this.textarea_result = textarea_result;
        this.Encode_type = Encode_type;
        this.souce_text = souce_text.trim();
        this.input_encoding = input_encoding;
        this.ouput_encoding = ouput_encoding;
...
//                System.out.println(result);
    ...
    public Result Fuc_Rot8000(String name,String text) {
        try (InputStream inputStream = CtfknifeApplication.class.getResourceAsStream("js/rot8000.js");
            ...
            Object result = invocable.invokeFunction("rotTransform", text);
    ...
    public Result  Fuc_四方密码(String name, String text) throws FileNotFoundException, ScriptException, NoSuchMethodException {
        try{
            if(temp_key_1.length()!=25 || temp_key_2.length()!=25){
                return new Result(name,false,"key长度需为25!Key1长度："+temp_key_1.length()+" Key2长度："+temp_key_2.length());
            }
            String result=    Conn.ExEjs_3("js/FourSquareCipher.js","decode",text,temp_key_1,temp_key_2);
            return new Result(name,true,(String) result);
        } catch(Exception ex) {
            return new Result(name,false,ex.getMessage());
        }
    ...
    public Result  Fuc_棋盘密码_ADFGX_(String name, String sourceText){
        try {
            if(temp_key_1.length()!=25){
                return new Result(name,false,"编码表长度需要为25！当前长度："+temp_key_1.length());
            }
    ...
    public Result  Fuc_与佛论禅_v1_(String name, String sourceText) throws Exception {
        String result = Yufulunchan.decryptFoYue(sourceText);
        return new Result(name,false,result);

    }
    public Result  Fuc_如是我闻(String name, String sourceText) throws Exception {
        String result = Yufulunchan.decryptRuShiWoWen(sourceText);
        return new Result(name,false,result);
