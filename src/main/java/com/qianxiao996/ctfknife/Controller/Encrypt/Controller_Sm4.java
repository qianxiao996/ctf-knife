...
public class Controller_Sm4 {
    ...
    void func_decrypt(ActionEvent event) {
        ...
        if(!key.isEmpty()){
            ...
            if(new String(key_value).startsWith("Error:")){
                ...
                if (key_value.length>16){
                    key_value = Arrays.copyOfRange(key_value,0,16);
                }
                if(key_value.length!=16){
                    f_alert_informationDialog("提示","","key的长度必须为16比特！");
                    return;
                }
            }
        }else{
        ...
        byte[] encodedBytes = Sm4Decrypt(input_text, key_value,iv_value,mode_text,padding_text);
    ...
    void func_encrypt(ActionEvent event) {
        ...
        String mode_text = mode.getValue();
        String padding_text = padding_combox.getValue();
        byte[] encodedBytes = Sm4Encrypt(input_text, key_value,iv_value,mode_text,padding_text);
        String output_encoding_text = output_encoding.getValue();
        if(new String(encodedBytes).startsWith("Error:")){
            TextArea_Result.setText(new  String(encodedBytes));
            return;
    ...
    private byte[] Sm4Encrypt(byte[] plainText, byte[] key,byte[] iv,String mode,String padding) {
        try {
            String encrypt_type = "SM4";
    ...
    private byte[] Sm4Decrypt(byte[] plainText, byte[] key,byte[] iv,String mode,String padding) {
        try {
            String encrypt_type = "SM4";
            SecretKey secretKey = new SecretKeySpec(key, encrypt_type);
            Cipher cipher = Cipher.getInstance(encrypt_type+"/"+mode+"/"+padding, "BC");
